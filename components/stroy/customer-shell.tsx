...
const navigation = [
  ...
  { label: "Мои проекты", href: "/customer/projects", icon: FolderKanban },
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="mx-auto flex min-h-screen max-w-[1720px]">
        <aside className="sticky top-0 hidden h-screen w-[232px] shrink-0 flex-col border-r border-border bg-card px-4 py-5 xl:flex">
          <Link href="/customer/dashboard" className="flex min-h-12 items-center px-2" aria-label="СтройВыбор — главная">
            <StroyVyborLogo className="w-[166px]" />
          </Link>

          <div className="mt-8 px-2 text-[11px] font-semibold uppercase tracking-[0.12em] text-muted-foreground">
            Меню
          </div>
          <nav className="mt-2 space-y-1" aria-label="Основная навигация заказчика">
            {navigation.map((item) => {
              ...
              return (
                <Link
                  ...
                  href={item.href}
                  className={[
                    "group flex min-h-11 items-center gap-3 rounded-xl px-3.5 text-sm font-semibold transition-colors",
        ...
        <div className="min-w-0 flex-1">
          <header className="sticky top-0 z-40 border-b border-border/80 bg-background/92 backdrop-blur-xl">
            <div className="flex min-h-[68px] items-center gap-3 px-4 sm:px-6 lg:px-8">
              <Link href="/customer/dashboard" className="flex shrink-0 items-center xl:hidden" aria-label="СтройВыбор — главная">
                <StroyVyborLogo className="w-[138px] sm:w-[154px]" />
              </Link>

              <Link
                href="/customer/contractors"
                className="mx-auto hidden min-h-10 w-full max-w-[560px] items-center gap-3 rounded-full border border-border bg-card px-4 text-sm text-muted-foreground shadow-sm transition hover:border-primary/25 hover:text-foreground md:flex"
                aria-label="Найти специалиста"
              >
                <Search className="h-4 w-4" aria-hidden="true" />
                <span>Найти услугу или специалиста</span>
              </Link>
              ...
              <div className="ml-auto flex shrink-0 items-center gap-2 sm:gap-3">
                ...
                <Link
                  ...
                  className="hidden min-h-10 items-center gap-2 rounded-xl px-2 transition hover:bg-secondary sm:flex"
      ...
      <nav className="fixed inset-x-0 bottom-0 z-50 border-t border-border bg-card/96 px-2 pb-[max(0.55rem,env(safe-area-inset-bottom))] pt-2 shadow-[0_-8px_28px_rgba(20,35,27,0.08)] backdrop-blur-xl xl:hidden" aria-label="Мобильная навигация">
        <div className="mx-auto grid max-w-xl grid-cols-5 items-end">
          ...
          <Link href="/customer/projects/new" className="mx-auto -mt-5 flex h-14 w-14 items-center justify-center rounded-full bg-primary text-primary-foreground shadow-[0_10px_28px_rgba(8,122,80,0.28)] transition hover:-translate-y-0.5" aria-label="Создать проект">
...
function MobileItem({ href, label, active, icon }: { href: string; label: string; active: boolean; icon: React.ReactNode }) {
  return (
    <Link href={href} className={["flex min-h-14 flex-col items-center justify-center gap-1 rounded-xl px-1 text-[10px] font-semibold transition", active ? "text-primary" : "text-muted-foreground"].join(" ")}>
      {icon}
