      ...
      <div className="app-container py-8 md:py-12">
        ...
        <section className="relative mt-5 overflow-hidden rounded-[2rem] border border-border bg-card p-6 shadow-[var(--shadow-soft)] md:p-8">
          ...
          <div className="relative flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
              ...
              <div>
                ...
                <div className="mt-4 flex flex-wrap gap-4 text-sm">
                  <div className="inline-flex items-center gap-2">
                    ...
                    <strong>{Number(company.rating ?? 0).toFixed(1)}</strong>
                    <span className="text-muted-foreground">{company.rating_count ?? 0} отзывов</span>
                  </div>
            ...
            {score && (
              <div className="min-w-[220px] rounded-[1.5rem] border border-primary/20 bg-secondary/55 p-5">
                <div className="flex items-center gap-2 text-primary">
                  <Sparkles className="h-4 w-4" />
                  <span className="text-xs font-black uppercase tracking-[0.12em]">StroySelect Score</span>
                </div>
                <div className="mt-3 flex items-end gap-2">
                  <span className="text-4xl font-black text-foreground">{score.score}</span>
          ...
          {company.description && (
            <p className="relative mt-6 max-w-4xl text-sm leading-7 text-muted-foreground md:text-base">
              {company.description}
            </p>
          )}
        </section>
        ...
        <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
          <div className="space-y-6">
            {score && (
              <SectionCard title="Почему такой StroySelect Score">
                ...
                <div className="mt-5 grid gap-3 sm:grid-cols-2">
                  {score.factors.map((factor) => (
                    <div key={factor.key} className="rounded-2xl border border-border bg-background/60 p-4">
                      <div className="flex items-center justify-between gap-3">
                        <p className="text-sm font-semibold text-foreground">{factor.label}</p>
                        <span className="text-sm font-black text-primary">{factor.points}/{factor.maxPoints}</span>
                      </div>
                ...
                {score.strengths.length > 0 && (
                  <div className="mt-5">
                    <p className="text-xs font-bold uppercase tracking-[0.1em] text-muted-foreground">Сильные стороны</p>
                    <div className="mt-2 flex flex-wrap gap-2">
                      {score.strengths.map((item) => (
                        <span key={item} className="rounded-full bg-emerald-50 px-3 py-1.5 text-xs font-semibold text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300">
                          {item}
                        </span>
                      ))}
                    </div>
            ...
            <SectionCard title="Специализации">
              {services.length > 0 ? (
            ...
            <SectionCard title={`Портфолио (${portfolio.length})`}>
              {portfolio.length > 0 ? (
            ...
            <SectionCard title={`Отзывы (${reviews.total})`}>
              {reviews.total > 0 ? (
                <>
                    ...
                    {reviews.reviews.map((review) => (
                      <article key={review.id} className="rounded-2xl border border-border bg-background p-5">
                        <div className="flex flex-wrap items-start justify-between gap-4">
                          <div>
                            <p className="font-semibold text-foreground">{getReviewerName(review.profiles)}</p>
                            {review.projects && (
                              <p className="mt-1 text-xs text-muted-foreground">{review.projects.title}</p>
                            )}
                          </div>
          ...
          <aside className="space-y-5 xl:sticky xl:top-24 xl:self-start">
            <SectionCard title="Данные компании">
              <div className="space-y-4">
                {company.founded_year && <InfoRow label="Год основания" value={String(company.founded_year)} />}
                {company.inn && <InfoRow label="ИНН" value={company.inn} />}
                {company.ogrn && <InfoRow label="ОГРН" value={company.ogrn} />}
                {company.contact_phone && (
                  <div>
                    <p className="text-xs text-muted-foreground">Телефон</p>
                    <a href={`tel:${company.contact_phone}`} className="mt-1 inline-flex items-center gap-2 text-sm font-semibold text-foreground hover:text-primary">
            ...
            <SectionCard title="География">
              {areas.length > 0 ? (
                <div className="space-y-3">
                  {areas.map((area, index) => (
                    <div key={`${area.city}-${index}`} className="rounded-xl bg-secondary/50 p-3">
                      <div className="flex items-center gap-2 text-sm font-semibold text-foreground">
                        <MapPin className="h-4 w-4 text-primary" />
                        {area.city}
                      </div>
                      {area.region && <p className="mt-1 pl-6 text-xs text-muted-foreground">{area.region}</p>}
                    </div>
                  ))}
                </div>
              ) : (
            ...
            <SectionCard title="Бюджет проектов">
            ...
            {score && score.improvements.length > 0 && (
              <SectionCard title="Что можно усилить">
