...
export default async function CustomerMaterialsPage({params,searchParams}:Props){const [{id},query]=await Promise.all([params,searchParams]);return <><ProjectMaterialsPage projectId={id} role="customer" query={query}/><MaterialOrderPanel projectId={id} role="customer" query={query}/><MaterialDeliveryPanel projectId={id} role="customer" query={query}/></>}
