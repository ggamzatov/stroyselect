...
async function insertNotification(input:{userId:string;projectId:string;type:string;title:string;body:string;url:string;key:string}){const result=await db.query<{id:string}>(`INSERT INTO public.notifications(user_id,notification_type,title,body,project_id,url,metadata,deduplication_key) VALUES($1::uuid,$2,$3,$4,$5::uuid,$6,'{}'::jsonb,$7) ON CONFLICT DO NOTHING RETURNING id`,[input.userId,input.type,input.title,input.body,input.projectId,input.url,input.key]);const id=result.rows[0]?.id;if(!id)return false;const pref=await db.query<{email_enabled:boolean;email:string|null}>(`SELECT COALESCE(np.email_enabled,true) AS email_enabled,p.email FROM public.profiles p LEFT JOIN public.notification_preferences np ON np.user_id=p.id WHERE p.id=$1::uuid`,[input.userId]);const settings=pref.rows[0];if(settings?.email_enabled&&settings.email)await db.query(`INSERT INTO public.notification_delivery_queue(notification_id,channel,recipient) VALUES($1::uuid,'email',$2) ON CONFLICT DO NOTHING`,[id,settings.email]);return true}

async function deliverQueuedEmail(){const rows=await db.query<{id:string;recipient:string;attempts:number;title:string;body:string|null;url:string|null}>(`SELECT q.id,q.recipient,q.attempts,n.title,n.body,n.url FROM public.notification_delivery_queue q JOIN public.notifications n ON n.id=q.notification_id WHERE q.status IN ('pending','failed') AND q.next_attempt_at<=now() AND q.attempts<6 ORDER BY q.created_at ASC LIMIT 50`);let sent=0,failed=0;for(const row of rows.rows){await db.query(`UPDATE public.notification_delivery_queue SET status='processing',attempts=attempts+1 WHERE id=$1::uuid`,[row.id]);try{const link=row.url?`${getAppBaseUrl()}${row.url}`:null;const html=`<div style="font-family:Arial,sans-serif;max-width:640px;margin:auto"><h2>${escapeHtml(row.title)}</h2>${row.body?`<p>${escapeHtml(row.body)}</p>`:""}${link?`<p><a href="${escapeHtml(link)}">Открыть в СтройВыбор</a></p>`:""}<p style="color:#777;font-size:12px">Это автоматическое уведомление СтройВыбор.</p></div>`;const response=await sendTransactionalEmail({to:row.recipient,subject:row.title,html});if(!response.success)throw new Error(response.message);await db.query(`UPDATE public.notification_delivery_queue SET status='sent',sent_at=now(),last_error=NULL WHERE id=$1::uuid`,[row.id]);sent++}catch(error){const delay=Math.min(360,Math.max(5,2**Math.min(8,Number(row.attempts)||0)));await db.query(`UPDATE public.notification_delivery_queue SET status='failed',last_error=$2,next_attempt_at=now()+($3::text||' minutes')::interval WHERE id=$1::uuid`,[row.id,error instanceof Error?error.message:String(error),delay]);failed++}}return{sent,failed}}
