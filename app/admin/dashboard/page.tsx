...
export default async function AdminDashboardPage() {
  const result = await db.query<{
    ...
    `
      SELECT
        (SELECT COUNT(*) FROM public.contractor_companies WHERE verification_status = 'pending') AS pending_count,
        (SELECT COUNT(*) FROM public.contractor_companies WHERE verification_status = 'verified') AS verified_count,
        (SELECT COUNT(*) FROM public.contractor_companies WHERE verification_status = 'rejected') AS rejected_count,
  ...
  const usersCount = Number(counts?.users_count ?? 0);
        ...
        <div className="relative flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground">
              Контролируйте подрядчиков, пользователей и ключевые процессы СтройВыбора из одного рабочего пространства.
            </p>
          </div>
          {pendingCount > 0 && (
            <Link
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_360px]">
        <section className="rounded-[1.75rem] border border-border bg-card p-6 shadow-[var(--shadow-soft)] md:p-7">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <p className="text-sm font-semibold text-primary">Модерация</p>
              <h2 className="mt-1 text-2xl font-bold tracking-tight text-foreground">Требуют внимания</h2>
              <p className="mt-2 text-sm leading-6 text-muted-foreground">
                Проверяйте профили подрядчиков перед предоставлением доступа к опубликованным проектам.
              </p>
            </div>
            ...
            {pendingCount > 0 ? (
              <div className="rounded-[1.4rem] border border-amber-200 bg-amber-50/70 p-5 dark:border-amber-900/50 dark:bg-amber-950/20">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <p className="font-bold text-foreground">Новые заявки подрядчиков</p>
                    <p className="mt-1 text-sm text-muted-foreground">
                      На проверке находится <strong className="text-foreground">{pendingCount}</strong>{" "}
                      {formatApplicationCount(pendingCount)}
                    </p>
                  </div>
                  <Link href="/admin/contractors?status=pending" className="inline-flex shrink-0 items-center gap-2 text-sm font-semibold text-primary">
                    Открыть
                    <ArrowRight className="h-4 w-4" />
                  </Link>
                </div>
              ...
              <div className="rounded-[1.4rem] border border-dashed border-border bg-background/60 p-8 text-center">
                ...
                <p className="mt-3 font-semibold text-foreground">Всё проверено</p>
                <p className="mt-1 text-sm text-muted-foreground">Новых заявок подрядчиков сейчас нет.</p>
              </div>
            )}
          </div>
        </section>
...
function QuickLink({ href, title, description, icon }: { href: string; title: string; description: string; icon: React.ReactNode }) {
  return (
    <Link href={href} className="group flex items-center gap-3 rounded-2xl border border-transparent p-3 transition hover:border-border hover:bg-secondary/50">
