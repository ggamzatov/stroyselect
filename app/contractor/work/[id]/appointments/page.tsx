...
export default async function ContractorProjectAppointmentsPage({ params }: Props) {
  const { id } = await params;
  const appointments = await getProjectAppointmentsForParticipant(id);
  return <ProjectAppointmentsPanel projectId={id} role="contractor" appointments={appointments} />;
}
