...
if (!session.includes('process.env.NODE_ENV === "production" && !allowInsecureE2E')) failures.push("session cookie must remain Secure in production except explicit loopback E2E");
...
if (!session.includes('headerStore.get("x-forwarded-host")') || !session.includes('headerStore.get("host")')) failures.push("E2E session override must be validated from request host");
