...
export async function processVerifiedYooKassaPayment(provider:YooPayment,providerPaymentId:string):Promise<Response>{
  if(provider.metadata?.payment_scope==="project_stage"||provider.metadata?.payment_intent_id)return processProjectPayment(provider,providerPaymentId);
  if(provider.metadata?.payment_scope==="ad_order"||provider.metadata?.local_ad_payment_id)return processAdPayment(provider,providerPaymentId);
  if(provider.metadata?.payment_scope==="material_order"||provider.metadata?.local_material_payment_id)return processMaterialPayment(provider,providerPaymentId);
...
async function processProjectPayment(provider:YooPayment,providerPaymentId:string){
  ...
  try{
    ...
    if(!local){await client.query("ROLLBACK");return Response.json({ok:true,ignored:"unknown_project_payment"});}

    if(provider.metadata?.payment_intent_id&&provider.metadata.payment_intent_id!==local.id){await client.query("ROLLBACK");return Response.json({ok:false,error:"metadata_mismatch"},{status:409});}
    if(provider.metadata?.project_id&&provider.metadata.project_id!==local.project_id){await client.query("ROLLBACK");return Response.json({ok:false,error:"project_mismatch"},{status:409});}
    if(provider.metadata?.stage_id&&provider.metadata.stage_id!==local.stage_id){await client.query("ROLLBACK");return Response.json({ok:false,error:"stage_mismatch"},{status:409});}
    ...
    if(provider.status!=="succeeded"||provider.paid===false){
      ...
      await client.query("COMMIT");
      return Response.json({ok:true,ignored:"not_succeeded"});
    }
    ...
    if(local.status!=="awaiting_payment"){
      await client.query("ROLLBACK");
      return Response.json({ok:false,error:"payment_state_mismatch"},{status:409});
    }
...
async function processAdPayment(provider:YooPayment,providerPaymentId:string){
  ...
  try{
    ...
    `,[providerPaymentId]);
    const local=localResult.rows[0];
    if(!local){await client.query("ROLLBACK");return Response.json({ok:true,ignored:"unknown_ad_payment"});}
    ...
    if(provider.status!=="succeeded"||provider.paid===false){await client.query("COMMIT");return Response.json({ok:true,ignored:"not_succeeded"});}
    if(local.status==="succeeded"){await client.query("COMMIT");return Response.json({ok:true,idempotent:true});}
    ...
    if(provider.metadata?.local_ad_payment_id&&provider.metadata.local_ad_payment_id!==local.id){await client.query("ROLLBACK");return Response.json({ok:false,error:"metadata_mismatch"},{status:409});}
    if(provider.metadata?.ad_order_id&&provider.metadata.ad_order_id!==local.order_id){await client.query("ROLLBACK");return Response.json({ok:false,error:"order_mismatch"},{status:409});}
    if(local.order_status!=="awaiting_payment"){
      await client.query("ROLLBACK");
      return Response.json({ok:false,error:"order_state_mismatch"},{status:409});
    }
...
async function processMaterialPayment(provider:YooPayment,providerPaymentId:string){
  ...
  try{
    ...
    `,[providerPaymentId]);
    const local=localResult.rows[0];
    if(!local){await client.query("ROLLBACK");return Response.json({ok:true,ignored:"unknown_material_payment"});}
    ...
    if(provider.metadata?.local_material_payment_id&&provider.metadata.local_material_payment_id!==local.id){await client.query("ROLLBACK");return Response.json({ok:false,error:"metadata_mismatch"},{status:409});}
    if(provider.metadata?.material_order_id&&provider.metadata.material_order_id!==local.order_id){await client.query("ROLLBACK");return Response.json({ok:false,error:"order_mismatch"},{status:409});}
    if(local.order_status!=="awaiting_payment"){
      await client.query("ROLLBACK");
      return Response.json({ok:false,error:"order_state_mismatch"},{status:409});
    }
...
async function processSubscriptionPayment(provider:YooPayment,providerPaymentId:string){
  ...
  try{
    ...
    `,[providerPaymentId]);
    const local=localResult.rows[0];
    if(!local){await client.query("ROLLBACK");return Response.json({ok:true,ignored:"unknown_payment"});}
    ...
    if(provider.metadata?.local_subscription_payment_id&&provider.metadata.local_subscription_payment_id!==local.id){await client.query("ROLLBACK");return Response.json({ok:false,error:"metadata_mismatch"},{status:409});}
