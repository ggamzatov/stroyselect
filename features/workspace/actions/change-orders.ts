...
const changeSchema = z.object({
  ...
  amountDelta: z.coerce.number().min(-1e9).max(1e9),
...
export async function createChangeOrder(formData: FormData) {
  ...
  const limited = await limitMutation(
    `finance:change-create:${parsed.data.projectId}`,
    userId,
    8,
    300,
    300
  ...
  try {
    ...
    const hold = await lockAndGetHoldError(client, parsed.data.projectId);
    if (hold) {
      await client.query("ROLLBACK");
      return { success: false, message: hold };
    }
    ...
    changeId = insertResult.rows[0]?.id;
    created = Boolean(changeId);
    ...
    if (!changeId) {
      const existingResult = await client.query<{ id: string }>(
        `
          ...
          LIMIT 1
        `,
        [parsed.data.projectId, parsed.data.idempotencyKey]
      );
      changeId = existingResult.rows[0]?.id;
    }

    if (!changeId) throw new Error("Не удалось определить change order после idempotent insert");
  ...
  if (created) {
    await notifyProjectParticipant({
      ...
      title: "Новый запрос изменения",
      body: `${parsed.data.title} · ${signedMoney(parsed.data.amountDelta)}`,
      customerUrl: `/customer/work/${parsed.data.projectId}/changes`,
      contractorUrl: `/contractor/work/${parsed.data.projectId}/changes`,
      deduplicationKey: `change-order-created:${changeId}`,
      metadata: { change_order_id: changeId },
    });
  }
  ...
  return {
    success: true,
    message: created
      ? "Изменение отправлено заказчику на согласование"
      : "Этот запрос изменения уже был сохранён",
  };
}
...
export async function decideChangeOrder(formData: FormData) {
  ...
  const limited = await limitMutation(
    `finance:change-decision:${parsed.data.changeOrderId}`,
    userId,
    6,
    300,
    300
  ...
  await notifyProjectParticipant({
    ...
    customerUrl: `/customer/work/${parsed.data.projectId}/changes`,
    contractorUrl: `/contractor/work/${parsed.data.projectId}/changes`,
    deduplicationKey: `change-order-${parsed.data.decision}:${parsed.data.changeOrderId}`,
    metadata: { change_order_id: parsed.data.changeOrderId },
  });
...
export async function cancelChangeOrder(formData: FormData) {
  ...
  const limited = await limitMutation(
    `finance:change-cancel:${projectId}`,
    userId,
    10,
    300,
    300
...
export async function recordProjectPayment(formData: FormData) {
  ...
  if (!parsed.success) return { success: false, message: "Проверьте сумму, этап и дату платежа" };

  const userId = await getCurrentSessionUserId();
  if (!userId) return { success: false, message: "Необходимо войти" };

  const limited = await limitMutation(
    `finance:payment:${parsed.data.projectId}`,
    userId,
    10,
    300,
    300
  ...
  try {
    ...
    const existingResult = await client.query<{ id: string }>(
      `
        ...
        WHERE project_id=$1::uuid AND idempotency_key=$2::uuid
        LIMIT 1
      `,
      [parsed.data.projectId, parsed.data.idempotencyKey]
    );
    ...
    if (!paymentId) {
      ...
      if (!budget) throw new Error("Project not found");
      if (budget.risk_hold) {
        await client.query("ROLLBACK");
        return { success: false, message: holdMessage(budget.risk_hold_reason) };
      }

      const total = Number(budget.contract_total);
      const remaining = total - Number(budget.paid_total);
      ...
      if (parsed.data.stageId) {
        const stageResult = await client.query<{
          ...
          `
            ...
            WHERE ps.id=$1 AND ps.project_id=$3
          `,
          [parsed.data.stageId, total, parsed.data.projectId]
        );
        ...
        const left = Math.max(0, Number(stage.planned) - Number(stage.paid));
        if (parsed.data.amount > left + 0.005) {
          await client.query("ROLLBACK");
          return {
            success: false,
            message: `Платёж превышает остаток по этапу (${money(left)})`,
          };
        }
      }
      ...
      if (!paymentId) {
        ...
        paymentId = concurrentResult.rows[0]?.id;
      }

      if (!paymentId) throw new Error("Не удалось определить payment после idempotent insert");
  ...
  if (!paymentId) return { success: false, message: "Не удалось определить платёж" };
  ...
  if (created) {
    await notifyProjectParticipant({
      ...
      title: "Зафиксирован платёж по проекту",
      body: `${money(parsed.data.amount)} · ${parsed.data.paidAt}`,
      customerUrl: `/customer/work/${parsed.data.projectId}/changes`,
      contractorUrl: `/contractor/work/${parsed.data.projectId}/changes`,
      deduplicationKey: `payment-recorded:${paymentId}`,
      metadata: {
        payment_id: paymentId,
        stage_id: parsed.data.stageId ?? null,
        amount: parsed.data.amount,
  ...
  return {
    success: true,
    message: created ? "Платёж добавлен" : "Этот платёж уже был сохранён",
  };
}
...
async function limitMutation(
  ...
  return result.allowed ? null : { success: false, message: rateLimitMessage(result) };
}
...
async function lockAndGetHoldError(client: PoolClient, projectId: string) {
  ...
  return project.risk_hold ? holdMessage(project.risk_hold_reason) : null;
}
...
function revalidate(id: string) {
  revalidatePath(`/customer/work/${id}/changes`);
  revalidatePath(`/contractor/work/${id}/changes`);
  revalidatePath(`/customer/work/${id}`);
  revalidatePath(`/contractor/work/${id}`);
}
