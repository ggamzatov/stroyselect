...
export async function updateProjectStageStatus(
  ...
  try {
    ...
    const projectResult = await client.query<ProjectRow>(
      `
        SELECT id,status::text AS status,risk_hold,risk_hold_reason
    ...
    if (project.risk_hold) {
      await client.query("ROLLBACK");
      return { success: false, message: holdMessage(project.risk_hold_reason) };
    }
    ...
    if (action === "start") {
      ...
      if (project.status === "contractor_selected") {
        ...
        if (stageCount === 0 || totalWeight !== 100) {
          ...
          return {
            success: false,
            message: `До начала первого этапа план проекта должен быть распределён на 100%. Сейчас распределено ${totalWeight}%.`,
          };
        }

        // First move the project through its guarded lifecycle transition. The DB
        // verifies the signed current contract and a complete 100% stage plan.
        // Only after that may an individual stage become in_progress.
        const startedProject = await client.query(
          `
            UPDATE public.projects
            SET status='in_progress',work_started_at=COALESCE(work_started_at,now()),updated_at=now()
            WHERE id=$1::uuid
              AND selected_contractor_id=$2::uuid
        ...
        if (!startedProject.rowCount) {
          throw new Error("Не удалось перевести проект в работу");
        }
      ...
      const updatedStage = await client.query(
        `
          ...
          SET status='in_progress',actual_started_at=now(),actual_completed_at=NULL,
              submitted_for_review_at=NULL,reviewed_at=NULL,reviewed_by=NULL,
              customer_review_comment=NULL,updated_at=now()
          WHERE id=$1::uuid AND project_id=$2::uuid AND status='planned'
          RETURNING id
        `,
        [stageId, projectId]
      );
      if (!updatedStage.rowCount) throw new Error("Не удалось начать этап");
      ...
      const submittedStage = await client.query(
        `
          UPDATE public.project_stages
          SET status='awaiting_review',submitted_for_review_at=now(),customer_review_comment=NULL,
              reviewed_at=NULL,reviewed_by=NULL,updated_at=now()
          WHERE id=$1::uuid AND project_id=$2::uuid AND status='in_progress'
          RETURNING id
        `,
        [stageId, projectId]
      );
      if (!submittedStage.rowCount) throw new Error("Не удалось отправить этап на проверку");
      ...
      notification = {
        ...
        body: `Подрядчик завершил этап «${stage.title}» и отправил его на проверку. Выплата подрядчику будет разрешена только после вашего подтверждения.`,
      };
      ...
      const resumedStage = await client.query(
        `
          UPDATE public.project_stages
          SET status='in_progress',submitted_for_review_at=NULL,reviewed_at=NULL,
              reviewed_by=NULL,updated_at=now()
          WHERE id=$1::uuid AND project_id=$2::uuid AND status='revision_required'
          RETURNING id
        `,
        [stageId, projectId]
      );
      if (!resumedStage.rowCount) throw new Error("Не удалось возобновить этап");
