...
type PaymentRow = { id:string; recorded_by:string; stage_id:string|null; amount:string|number; paid_at:Date|string; note:string|null; created_at:Date|string };
...
export async function getProjectBudgetControl(projectId: string) {
  ...
  const [changesResult,paymentsResult,stagesResult]=await Promise.all([
    ...
    db.query<PaymentRow>(`SELECT id,recorded_by,stage_id,amount,paid_at,note,created_at FROM public.project_payments WHERE project_id=$1 ORDER BY paid_at DESC,created_at DESC`,[projectId]),
  ...
  const paidTotal=paymentsResult.rows.reduce((s,i)=>s+toNumber(i.amount),0);

  const paymentsByStage=new Map<string,number>();
  for(const payment of paymentsResult.rows){if(payment.stage_id) paymentsByStage.set(payment.stage_id,(paymentsByStage.get(payment.stage_id)??0)+toNumber(payment.amount));}
  ...
  return {
    ...
    payments:paymentsResult.rows.map(row=>({id:row.id,recordedBy:row.recorded_by,stageId:row.stage_id,amount:toNumber(row.amount),paidAt:toDateString(row.paid_at),note:row.note,createdAt:toIso(row.created_at)})),
  };
}
