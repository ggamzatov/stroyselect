...
export function ContractorCompanyForm({ categories, cities, company }: Props) {
  ...
  function handleAvailabilityChange(value: boolean) {
    ...
    if (!company) {
      setMessage("Статус доступности будет сохранён вместе с анкетой профиля.");
      return;
    }
  ...
  function onInvalid() {
    ...
    const validationMessage = firstError && typeof firstError === "object" && "message" in firstError && typeof firstError.message === "string"
      ? firstError.message
    ...
    <div className="space-y-6">
      ...
      {editingVerifiedProfile && (
        <div className="rounded-[1.25rem] border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-900">
          Профиль подтверждён. Вы можете изменить услуги, города работы, реквизиты и контакты. После сохранения обновлённая анкета будет автоматически направлена на повторную проверку.
        </div>
      )}
      ...
      <form onSubmit={handleSubmit(onSave, onInvalid)} className="space-y-6">
        ...
        <div className="sticky bottom-4 z-20 rounded-[1.5rem] border border-border bg-card/95 p-4 shadow-[var(--shadow-card)] backdrop-blur">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-sm font-semibold text-foreground">Анкета компании</p>
              <p className="mt-1 text-xs text-muted-foreground">
                {formLocked ? "Анкета временно заблокирована на период проверки. Доступность для новых проектов можно менять отдельно." : isDirty ? "Есть несохранённые изменения." : "Все изменения сохранены."}
              </p>
            </div>
            <div className="flex flex-wrap gap-3">
              {!formLocked && (
                <button type="submit" disabled={isSaving || isSubmitting} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-primary px-5 text-sm font-semibold text-primary-foreground shadow-[0_8px_20px_rgba(107,70,50,0.16)] transition hover:-translate-y-0.5 hover:bg-[#5c3b2a] disabled:pointer-events-none disabled:translate-y-0 disabled:opacity-50">
                  {isSaving ? <><Loader2 className="h-4 w-4 animate-spin" />Сохраняем...</> : <><Save className="h-4 w-4" />{editingVerifiedProfile ? "Сохранить изменения" : "Сохранить анкету"}</>}
                </button>
              )}
              {company && (status === "draft" || status === "rejected") && (
