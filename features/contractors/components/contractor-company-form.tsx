        ...
        <div className="sticky bottom-4 z-20 rounded-[1.5rem] border border-border bg-card/95 p-4 shadow-[var(--shadow-card)] backdrop-blur">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-wrap gap-3">
              {!formLocked && <button type="submit" disabled={isSaving || isSubmitting} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-primary px-5 text-sm font-semibold text-primary-foreground shadow-[0_8px_20px_rgba(8,122,80,0.20)] transition hover:-translate-y-0.5 hover:bg-[#076c47] disabled:pointer-events-none disabled:translate-y-0 disabled:opacity-50">{isSaving ? <><Loader2 className="h-4 w-4 animate-spin" />Сохраняем...</> : <><Save className="h-4 w-4" />{editingVerifiedProfile ? "Сохранить изменения" : "Сохранить анкету"}</>}</button>}
