...
export async function getAvailableProject(projectId: string) {
  ...
  const companyResult = await db.query<CompanyRow>(`
    SELECT id, verification_status, accepts_new_projects
  ...
  try {
    const projectResult = await db.query<ProjectRow>(`
      SELECT p.id, p.category_id, p.title, p.description, p.property_type,
             p.region, p.city, p.address, p.budget_min, p.budget_max,
             p.desired_start_date, p.desired_end_date, p.status, p.published_at,
    ...
    const bidResult = await db.query<BidRow>(`
      SELECT id, price, duration_days, message, proposed_start_date, status,
             created_at, updated_at, scope_summary, materials_summary, exclusions,
             payment_terms, warranty_months, price_includes_materials, completeness_score
    ...
    return {
      ...
      existingBid: bid ? {
        ...
        message: bid.message ?? "",
        proposed_start_date: toNullableDateString(bid.proposed_start_date),
        status: bid.status,
        created_at: toIsoString(bid.created_at),
        updated_at: toIsoString(bid.updated_at),
        scope_summary: bid.scope_summary ?? "",
        materials_summary: bid.materials_summary ?? "",
        exclusions: bid.exclusions ?? "",
        payment_terms: bid.payment_terms ?? "",
        warranty_months: bid.warranty_months ?? 0,
        price_includes_materials: bid.price_includes_materials,
        completeness_score: bid.completeness_score,
      } : null,
    };
  } catch (error) {
