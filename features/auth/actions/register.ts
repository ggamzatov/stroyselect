...
export async function registerUser(input:RegisterInput):Promise<RegisterState>{
 ...
 const email=parsed.data.email.trim().toLowerCase();const ip=await getRequestIp();const h=await headers();const userAgent=h.get("user-agent");
 const [ipLimit,emailLimit]=await Promise.all([enforceRateLimit({scope:"register:ip",identity:ip,limit:5,windowSeconds:3600,blockSeconds:3600}),enforceRateLimit({scope:"register:email-ip",identity:`${email}\n${ip}`,limit:3,windowSeconds:3600,blockSeconds:3600})]);const rejected=!ipLimit.allowed?ipLimit:!emailLimit.allowed?emailLimit:null;if(rejected)return{success:false,message:rateLimitMessage(rejected)};
 ...
 try{await client.query("BEGIN");const userResult=await client.query<{id:string}>(`INSERT INTO public.users(id,email,password_hash,email_confirmed_at,raw_user_meta_data,is_active) VALUES(gen_random_uuid(),$1::text,$2::text,NULL,$3::jsonb,true) RETURNING id`,[email,passwordHash,JSON.stringify({role,first_name:firstName,last_name:lastName??null})]);const userId=userResult.rows[0]?.id;if(!userId)throw new Error("Не удалось создать пользователя");await client.query(`INSERT INTO public.profiles(id,role,first_name,last_name,email,is_blocked) VALUES($1::uuid,$2,$3,$4,$5::text,false)`,[userId,role,firstName,lastName??null,email]);
  for(const documentType of ["terms","privacy_policy","personal_data_consent"]){await client.query(`INSERT INTO public.user_legal_acceptances(user_id,document_type,document_version,ip_address,user_agent,evidence) VALUES($1::uuid,$2,$3,$4::inet,$5,$6::jsonb)`,[userId,documentType,LEGAL_VERSION,ip||null,userAgent,JSON.stringify({source:"registration",affirmativeAction:"checkbox",version:LEGAL_VERSION})])}
  verificationToken=await issueAccountEmailToken(userId,"verify_email",60,client);await client.query("COMMIT")
 }catch(error){await client.query("ROLLBACK");console.error("Ошибка локальной регистрации:",error);return{success:false,message:"Не удалось создать учётную запись"}}finally{client.release()}
