...
export async function saveContractorReview(
  ...
  if (!parsed.success) {
    return { success: false, message: parsed.error.issues[0]?.message ?? "Проверьте отзыв" };
  }
  ...
  if (profile.role !== "customer") {
    return { success: false, message: "Оставить отзыв может только заказчик" };
  }
  ...
  if (project.status !== "completed") {
    return { success: false, message: "Отзыв можно оставить только после завершения проекта" };
  }
  if (!project.selected_contractor_id) {
    return { success: false, message: "Подрядчик проекта не найден" };
  }
  ...
  try {
      ...
      const result = await client.query<{ id: string }>(
        `
          INSERT INTO public.contractor_reviews (
            project_id, contractor_id, customer_id,
    ...
    return {
      success: false,
      message: created ? "Не удалось опубликовать отзыв" : "Не удалось обновить отзыв",
    };
  } finally {
    client.release();
  }
