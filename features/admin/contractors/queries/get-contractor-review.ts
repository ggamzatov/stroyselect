...
export async function getContractorReview(contractorId: string) {
  try {
    ...
    const [ownerResult,logsResult,historyResult,registryResult,matchResult]=await Promise.all([
    ...
    return {company,owner,logs,profileChanges,registryChecks,openMatches:matchResult.rows};
