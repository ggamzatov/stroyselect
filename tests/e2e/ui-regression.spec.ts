...
test.describe("UI regression",()=>{test.describe.configure({mode:"serial"});test.beforeEach(()=>test.skip(!fixtureAvailable,"Run npm run e2e:seed to provision fixtures"));
 ...
 test("customer workspace tabs keep layout and localization",async({page})=>{await login(page,customer!);for(const suffix of ["","/appointments","/contract","/changes","/documents","/issues","/disputes"]){await verify(page,`/customer/work/${workspaceProjectId}${suffix}`)}});
