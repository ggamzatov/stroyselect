...
test.describe("production finance lifecycle",()=>{
  ...
  test("authoritative webhook funds safe deals before admin payout or refund",async({page,request})=>{
    ...
    let payoutArticle=page.locator("article").filter({hasText:"E2E finance payout stage"}).first();
    ...
    payoutArticle=page.locator("article").filter({hasText:"E2E finance payout stage"}).first();
    ...
    let refundArticle=page.locator("article").filter({hasText:"E2E finance refund stage"}).first();
    ...
    refundArticle=page.locator("article").filter({hasText:"E2E finance refund stage"}).first();
